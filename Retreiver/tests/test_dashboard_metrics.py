...
def test_metrics_quality_table_renders_all_180_rows_and_formats_metrics_to_three_decimals():
    app = Path(__file__).resolve().parents[1] / "web" / "app.js"
    js = f"""
const fs=require('fs'), vm=require('vm');
const elements={{}};
function el(id) {{
  return elements[id] ||= {{id,value:'all',textContent:'',innerHTML:'',classList:{{toggle(){{}}}},addEventListener(){{}},querySelectorAll(){{return [];}},setAttribute(){{}}}};
}}
const context={{console,document:{{getElementById:el,querySelectorAll(){{return [];}},addEventListener(){{}},body:{{insertAdjacentHTML(){{}}}}}},window:{{}},location:{{hash:'#metrics'}},history:{{replaceState(){{}}}},fetch:async()=>({{ok:true,json:async()=>{{}},text:async()=>''}}),setTimeout(){{}}}};
vm.createContext(context);
const code=fs.readFileSync({str(app)!r},'utf8').split('loadOptions().then(refresh)')[0];
vm.runInContext(code + `
const rows=Array.from({{length:180}},(_,i)=>({{
  sheet:'chunker-'+i,embedding:'embedding',store:'store',reranker:'none',
  winner_score:180-i,evaluated_queries:5,recall_at_1:.123456,recall_at_3:.234567,
  recall_at_5:.345678,recall_at_10:.456789,mrr:.567891,precision_at_5:.678912,ndcg_at_5:.789123,
  avg_first_relevant_rank:1.23456,no_hit_queries:7,avg_latency_seconds:.034567,
}}));
table=(_el, renderedRows, cols)=>{{
  globalThis.__qualityRows=renderedRows;
  if (renderedRows.length !== 180) return;
  const metricKeys=['recall_at_1','recall_at_3','recall_at_5','recall_at_10','mrr','precision_at_5','ndcg_at_5','avg_first_relevant_rank','no_hit_queries','avg_latency_seconds'];
  globalThis.__formatted=Object.fromEntries(cols.filter(c=>metricKeys.includes(c.key)).map(c=>[c.key,c.render ? c.render(renderedRows[0],0) : String(renderedRows[0][c.key])]));
}};
renderEvaluation({{summary:rows}}, false);
globalThis.__winner=document.getElementById('qualityInsight').innerHTML;
`, context);
const expected={{recall_at_1:'0.123',recall_at_3:'0.235',recall_at_5:'0.346',recall_at_10:'0.457',mrr:'0.568',precision_at_5:'0.679',ndcg_at_5:'0.789',avg_first_relevant_rank:'1.235',no_hit_queries:'7',avg_latency_seconds:'0.035'}};
if (context.__qualityRows.length !== 180 || context.__qualityRows[0].sheet !== 'chunker-0' || !context.__winner.includes('chunker-0') || JSON.stringify(context.__formatted) !== JSON.stringify(expected)) {{
  console.error(JSON.stringify({{count:context.__qualityRows.length,first:context.__qualityRows[0],winner:context.__winner,formatted:context.__formatted}}));
  process.exit(1);
}}
"""
    proc = subprocess.run(["node", "-e", js], text=True, capture_output=True, timeout=10)
...
def test_grounding_audit_is_visibly_disabled_until_the_feature_is_active() -> None:
    ...
    assert re.search(
        r"\.advanced-grounding-actions button:disabled\{[^}]*background:rgba\(159,177,200,[^)]+\)[^}]*color:var\(--muted\)[^}]*box-shadow:none",
        styles,
    )
    assert ".tab-btn.feature-disabled" in styles


def test_grounding_actions_stay_natively_disabled_in_default_evaluated_mode() -> None:
    app = Path(__file__).resolve().parents[1] / "web" / "app.js"
    js = f"""
const fs=require('fs'), vm=require('vm');
const elements={{}};
...
const code=fs.readFileSync({str(app)!r},'utf8').split('loadOptions().then(refresh)')[0];
vm.runInContext(code + `
state.sourceCatalog={{datasets:[{{id:'dataset:wns-default',kind:'default'}}]}};
document.getElementById('runDataset').value='dataset:wns-default';
document.getElementById('runGroundtruth').value='groundtruth:official';
syncRunMode();
globalThis.__groundingDisabled=['runHallucinationBtn','runHallucinationLlmBtn'].every(id=>document.getElementById(id).disabled);
globalThis.__evaluationEnabled=!document.getElementById('runEvalBtn').disabled;
`, context);
if (!context.__groundingDisabled || !context.__evaluationEnabled) process.exit(1);
"""
    proc = subprocess.run(["node", "-e", js], text=True, capture_output=True, timeout=10)
    assert proc.returncode == 0, proc.stdout + proc.stderr


def test_hidden_dashboard_controls_cannot_be_overridden_by_grid_label_styles() -> None:
    styles = (Path(__file__).resolve().parents[1] / "web" / "styles.css").read_text(encoding="utf-8")

    assert "[hidden]{display:none!important}" in styles.replace(" ", "")
...
def test_frontend_assets_use_current_cache_key():
    ...
    assert "/app.js?v=20260724-canonical-dashboard-v5.4" in index
    assert "/styles.css?v=20260724-canonical-dashboard-v5.4" in index
    assert "/recommendation-visuals.js?v=20260724-canonical-dashboard-v5.4" in index
...
def test_run_pipeline_visibly_restates_the_global_source_context() -> None:
    ...
    assert 'id="runSourceContext"' in index
    app_text = app.read_text(encoding="utf-8")
    assert "renderRunSourceContext(datasetLabel, groundtruthLabel)" in app_text

    js = f"""
const fs=require('fs'),vm=require('vm');
const elements={{}};
function el(id) {{ return elements[id] ||= {{id,textContent:'',innerHTML:'',classList:{{toggle(){{}}}},addEventListener(){{}},querySelectorAll(){{return [];}},setAttribute(){{}}}}; }}
const context={{console,document:{{getElementById:el,querySelectorAll(){{return [];}},addEventListener(){{}},body:{{insertAdjacentHTML(){{}}}}}},window:{{}},location:{{hash:'#overview'}},history:{{replaceState(){{}}}},fetch:async()=>({{ok:true,json:async()=>{{}},text:async()=>''}}),setTimeout(){{}}}};
vm.createContext(context);
const code=fs.readFileSync({str(app)!r},'utf8').split('loadOptions().then(refresh)')[0];
vm.runInContext(code + `
renderRunSourceContext('Customer demo · 5 documents', 'Validated GT · 20 queries');
globalThis.__text=document.getElementById('runSourceContext').textContent;
`, context);
if (context.__text !== 'Running on: Customer demo · 5 documents · Ground truth: Validated GT · 20 queries · Change using the selectors at the top.') {{
  console.error(context.__text); process.exit(1);
}}
"""
    proc = subprocess.run(["node", "-e", js], text=True, capture_output=True, timeout=10)
    assert proc.returncode == 0, proc.stdout + proc.stderr
