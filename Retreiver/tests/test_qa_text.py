...
"""
LLM + RAG Service (adapter-first, config-driven)

- All LLM calls go ONLY through llm_client.create_completion(...)
- Prompts are resolved via prompt_manager, with prompt keys coming from config.yaml
- Retrieval via vector_service adapter when available; minimal pymilvus fallback retained
- No env reads here; config via source.config loaders
...
class LLMService:
    ...
    async def generate_response(
        ...
        if use_rag:
            ...
            #    print(source)
            if model=='llama3.3-70b-wns-airesearch-domain' or model=='meta/llama-3.3-70b-instruct':
...
root@ip-172-31-66-8:/home/General_Components/QA_Text/Retreiver#
root@ip-172-31-66-8:/home/General_Components/QA_Text/Retreiver#
root@ip-172-31-66-8:/home/General_Components/QA_Text/Retreiver# cat tests/test_qa_text.py
import os
import time
...
class LLMAccuracyEvaluator:
    ...
    async def call_llm_api(self, question, use_rag=True):
        url = "http://0.0.0.0:5010/llm/"
    ...
    def bertscore_f1(self, ref, cand):
        _, _, F1 = bert_score([cand], [ref], lang='en', verbose=False, device='cpu')
        return float(F1[0])
    ...
    async def evaluate(self, num_records=None):
        with open(self.file_path, newline='', encoding='utf-8') as csvfile:
            ...
            for i, row in enumerate(reader):
                if i == 0:
                    continue
                if num_records is not None and i >= num_records + 1:
                    break
                question, expected_answer = row[0], row[1]

                start_time = time.time()
                llm_answer = await self.call_llm_api(question)
                end_time = time.time()
                exec_time = end_time - start_time

                rouge = self.rouge_l(expected_answer, llm_answer)
                cosine = self.cosine_sim(expected_answer, llm_answer)
                bert_f1 = self.bertscore_f1(expected_answer, llm_answer)
                bleurt = self.bleurt_score(expected_answer, llm_answer)

                self.append_to_csv([question, expected_answer, llm_answer, rouge, cosine, bert_f1, bleurt, exec_time])
