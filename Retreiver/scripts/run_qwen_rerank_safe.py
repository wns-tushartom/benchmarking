...
def clean_text(value):
    ...
    max_chars = int(os.environ.get("RERANK_MAX_CHARS", "800"))
...
def rerank_file(path, out_dir, url, top_k):
    ...
    docs = [clean_text(h.get("paragraph") or "") for h in hits[:top_k]]
...
"retrieved_count": len(ranked),
        ...
        "rerank_max_chars": int(os.environ.get("RERANK_MAX_CHARS", "800")),
    })
    name = "{}_{}_{}_qwen3_4b_rerank_{}.json".format(
...
def main():
    ...
    ap.add_argument("--out-dir", default="data/reranker_smoke_qwen_sample")
    ap.add_argument("--top-k", type=int, default=5)
    ap.add_argument("--limit-artifacts", type=int, default=100)
    args = ap.parse_args()
    os.chdir(ROOT)
    url = os.environ.get("QWEN_RERANK_URL", "http://127.0.0.1:5000/rerank/qwen")
    artifacts = load_artifacts(args.retrieval_dir, args.limit_artifacts)
    print(f"artifacts={len(artifacts)} url={url} top_k={args.top_k} max_chars={os.environ.get('RERANK_MAX_CHARS', '800')}", flush=True)
    results = []
    errors = []
    for i, path in enumerate(artifacts, 1):
        try:
            result = rerank_file(path, args.out_dir, url, args.top_k)
            print(f"OK {i}/{len(artifacts)} seconds={result.get('rerank_seconds')} query={result.get('query')!r}", flush=True)
